...
def create_layer0(input_dim=551, n_jobs=28, n_est=224, verbose=1):
    ...
    ### linear models
    if ridge: reg_layer.append( ( "Ridge", Ridge(alpha=.5) ) )
    if lasso: reg_layer.append( ( "Lasso", Lasso(alpha=.1) ) )
    if elasticnet: reg_layer.append( ( "ElasticNet", ElasticNet(alpha=1., l1_ratio=0.5) ) )
    if huber: reg_layer.append( ( "Huber", HuberRegressor() ) )
    if ransac: reg_layer.append( ( "RansacRF", RANSACRegressor(base_estimator=RandomForestRegressor(n_estimators=n_est//4, n_jobs=n_jobs)) ) )
    if linearsvr: reg_layer.append( ( "LinearSVR", LinearSVR() ) )

    ### tree
    if tree: reg_layer.append( ( "Tree", DecisionTreeRegressor() ) )

    ### gbr
    if gbr: reg_layer.append( ( "GBR", GradientBoostingRegressor(loss='huber', learning_rate=0.02, n_estimators=n_est, subsample=0.65,
    ...
    ### adaboost
    if adarf: reg_layer.append( ( "AdaBoostRF", AdaBoostRegressor(base_estimator=RandomForestRegressor(n_estimators=n_est, n_jobs=n_jobs),
                                                              n_estimators=n_est//20, learning_rate=0.9) ) )
    if adaet: reg_layer.append( ( "AdaBoostExtraTrees", AdaBoostRegressor(base_estimator=ExtraTreesRegressor(n_estimators=n_est//2, bootstrap=True, n_jobs=n_jobs),
                                                                      n_estimators=n_est//20, learning_rate=0.9) ) )
    if adaxgb: reg_layer.append( ( "AdaBoostXGBoost", AdaBoostRegressor(base_estimator=XGBRegressor_ensembling(objective='reg:logistic',
    ...
    in enumerate(zip(max_depth_list, min_samples_split_list, max_features_list, min_samples_leaf_list)):
        if rf: reg_layer.append( ( "RF%d"%ix, RandomForestRegressor(n_estimators=n_est, max_depth=max_depth,
        ...
        if et: reg_layer.append( ( "ExtraTrees%d"%ix, ExtraTreesRegressor(n_estimators=n_est, max_depth=max_depth,
                                                                   min_samples_split=min_samples_split, criterion='mse',
                                                                   bootstrap=True, max_features=max_features,
                                                                   min_samples_leaf=min_samples_leaf, n_jobs=n_jobs) ) )
